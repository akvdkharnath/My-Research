...
poly_reg=PolynomialFeatures(degree=4)   #by defalult aslo it is 2 change the degree as u get the perfect curve at the end 
X_poly=poly_reg.fit_transform(X)      #converts the linear into polynomial type
# here we tranform the liner to polynomial type and from here we need to apply linear regression only   
lin_reg=LinearRegression()
lin_reg.fit(X_poly,Y)
z=lin_reg.predict(poly_reg.fit_transform(X))
#plot
plt.figure(1)
