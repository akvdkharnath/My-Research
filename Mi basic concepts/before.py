...
def ratio_of_chat(df):
	...
	opacity = 0.8
	plt.figure(5)
	plt.bar(user1_results[0],user1_results[1],color='r',label=dup_user_list[0])
	plt.bar(user2_results[0],user2_results[1],color='g',label=dup_user_list[1])
	plt.legend()
	plt.title('Analysis_Month_Vs_Chat')
	plt.xlabel('Months')
	plt.ylabel('Number of Messages')
...
def main():
	watsup_file = 'WhatsApp Chat with Algonox Soujanya.txt'
# Read WhatsApp file
